...
public class ApiPostController {
    ...
    @GetMapping("/{postId}")
    public ResponseEntity<PostDTO> getPost(@PathVariable(name = "postId") Long postId, Principal principal) {
        User currentUser = userService.getUserByUsername(principal.getName());
        if (currentUser == null) {
            return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        }
    ...
    @PutMapping("/{postId}")
    public ResponseEntity<Void> updatePost(@PathVariable(name = "postId") Long postId, @RequestBody Post post) {
        post.setId(postId);
